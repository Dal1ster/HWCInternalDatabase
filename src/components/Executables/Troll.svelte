...
<script lang="ts">
    ...
    const EVIL = {
        T: '/img/troll_e.jpg',
        T_E1: '/img/troll_e_e1.png',
        T_E2: '/img/troll_e_e2.png',
    } as const;

    const {
        loaded: allImagesLoaded,
        cache,
    } = trolled ? createLocalCache(EVIL) : createLocalCache(SAFE);
    ...
    async function onEpicTroll() {
        ...
        await Promise.all([
            hijackCache(),
            sfx.load('/sfx/trolled_h.mp3', 'sfx_s_trolling',  0),
            sfx.load('/sfx/trolled_ouch_short.mp3', 'sfx_s_ouch',  0),
            allImagesLoaded
        ])
    ...
    function ouch(e: MouseEvent) {
        ...
        sfx.stop('sfx_s_trolling');
        sfx.play('sfx_s_ouch' as any).then(() => hp > 0 && sfx.play('sfx_s_trolling' as any, true));
...
<div class="main" bind:this={div}>
    {#if !trolled}
        <img src={$cache[SAFE.T]} on:load={onTroll}/>
    {:else}
        <img class="t" src={$cache[EVIL.T]} on:load={onEpicTroll} />
        <div class="e e-l" bind:this={eyeLeft} on:mousedown={ouch}>
            <img src={$cache[EVIL.T_E1]}/>
        </div>
        <div class="e e-r" bind:this={eyeRight} on:mousedown={ouch}>
            <img src={$cache[EVIL.T_E2]}/>
        </div>
    {/if}
</div> 
